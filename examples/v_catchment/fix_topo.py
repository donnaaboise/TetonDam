...
topofile = 'vcatch_dem.asc'
topofile_new = 'vcatch_dem.topo'
...
l = f.readline()
ncols = np.fromstring(l.split()[1].strip(),sep=' ')[0].astype(int)
print(type(ncols))                
fnew.write("%s %s\n" % (l.split()[1],l.split()[0]))

l = f.readline()
nrows = np.fromstring(l.split()[1].strip(),sep=' ')[0].astype(int)
print(type(nrows))        
fnew.write("%s %s\n" % (l.split()[1],l.split()[0]))
